...
function CollaborationBar() {
    ...
    return (
        <div className={`${style.slider} w-full h-[88px] primary-color-bg overflow-hidden relative`}>
            <p className={`${style.collabsText} heading-6 text-white`}>Destekçilerimiz</p>
            <div className="flex items-center justify-start w-full h-full slider-track">
                <div className={`${style.scrollAnimation} flex items-center gap-16`}>
                    {extendedLogos.map((logo, index) => (
                        <img key={index} src={logo} alt={`Logo ${index}`} className="h-9 w-auto" />
                    ))}
                </div>
            </div>
        </div>
